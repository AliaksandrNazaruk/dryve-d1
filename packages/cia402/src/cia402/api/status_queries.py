"""Status and telemetry query mixin for CiA402Drive.
...
if TYPE_CHECKING:
    ...
    from .drive import CiA402DriveConfig
...
class StatusQueriesMixin:
    ...
    # Provided by CiA402Drive
    _cfg: CiA402DriveConfig
    ...
    async def is_homed(self) -> bool:
        ...
        Delegates to device hooks for vendor-specific homing checks.
        Falls back to statusword bit 12 (target reached after homing) as standard CiA 402.
        """
        if hasattr(self, '_hooks') and self._hooks is not None:
            result = await self._hooks.is_homed(self)
            if result is not None:
                return result
        # Standard CiA 402 fallback: no reliable generic homing check
        return False
